...
//	}
	...
	protected void init(boolean animated) {
		this.animated = animated;
		if(this.animated) {
			initAnimated();
		}
	...
	private void initAnimated() {
		this.loadAsset();
		final TextureRegion[][] tmp = texture.split(texture.getRegionWidth() / numPictures, texture.getRegionHeight()); // #8
		this.textureRegion = new TextureRegion[numPictures];
		int index = 0;
